-- ==========================================================
-- 1. CREATE SILVER TABLES (CLEANED DATA)
-- ==========================================================
...
-- ERP Category Table (Using 'q1v2' as per Bronze)
...
-- ==========================================================
-- 2. PROCEDURE FOR DATA TRANSFORMATION (BRONZE -> SILVER)
-- ==========================================================
...
BEGIN
    ...
    -- 1. crm_cust_info (Remove duplicates & normalize text)
    ...
    WHERE flag_last = 1;
    RAISE NOTICE '>> cst_info cleaned. Duration: %', clock_timestamp() - v_start_time;

    -- 2. crm_prd_info (Key split & Date correction)
    v_start_time := clock_timestamp();
    ...
    RAISE NOTICE '>> prd_info cleaned. Duration: %', clock_timestamp() - v_start_time;

    -- 3. crm_sales_details (Data type conversion & sales recalculation)
    v_start_time := clock_timestamp();
    ...
    RAISE NOTICE '>> sales_details cleaned. Duration: %', clock_timestamp() - v_start_time;
    ...
    SELECT REPLACE(cid, '-', ''), 
           CASE WHEN TRIM(cntry) = 'DE' THEN 'Germany' 
                ...
                ELSE COALESCE(NULLIF(TRIM(cntry), ''), 'n/a') END 
    FROM bronze.erp_loc_a101;
    RAISE NOTICE '>> loc_a101 cleaned. Duration: %', clock_timestamp() - v_start_time;
    ...
    RAISE NOTICE '>> cust_az12 cleaned. Duration: %', clock_timestamp() - v_start_time;
    ...
    RAISE NOTICE '>> cat_q1v2 loaded. Duration: %', clock_timestamp() - v_start_time;

    RAISE NOTICE '==========================================';
    RAISE NOTICE 'SILVER PROCESS COMPLETED. Total Duration: %', clock_timestamp() - v_batch_start_time;
    RAISE NOTICE '==========================================';

EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE '!!! AN ERROR OCCURRED IN THE SILVER LAYER !!!';
...
-- 2. Check Silver Results (Sample of 3 main tables)
